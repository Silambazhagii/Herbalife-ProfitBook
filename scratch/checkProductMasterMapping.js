...
// Existing master products from productsStore.js
      ...
      if (slMatch && !trimmed.includes('Delivery Charges') && !trimmed.includes('Total') && !trimmed.includes('SL.')) {
        ...
        if (numMatches.length >= 9) {
          ...
          if (!lineItemMap.has(sl)) {
            lineItemMap.set(sl, { sl, sku, description: cleanDesc, hsn, qty, unitPrice });
          }
        }
      }
    }
...
async function run() {
  ...
  console.log(`Extracted ${allExtractedItems.length} total product line items from all 10 PDFs.\n`);

  const uniqueSkusMap = new Map();
  allExtractedItems.forEach(item => {
    if (!uniqueSkusMap.has(item.sku)) {
      uniqueSkusMap.set(item.sku, item);
    }
  });

  console.log(`Found ${uniqueSkusMap.size} unique SKUs in uploaded invoices:\n`);
  
  const missingSkus = [];
  uniqueSkusMap.forEach((item, sku) => {
    const match = masterProducts.find(p => p.sku === sku);
    if (match) {
      console.log(`✅ SKU ${sku.padEnd(6)} -> Mapped to: "${match.name}" (MRP: ₹${match.mrp}, Volume: ${match.volume})`);
    } else {
      console.log(`❓ SKU ${sku.padEnd(6)} -> NOT in initial master: "${item.description}" (UnitPrice/MRP: ₹${item.unitPrice})`);
      missingSkus.push(item);
    }
  });

  console.log(`\nTotal missing SKUs to add to product master: ${missingSkus.length}`);
}
